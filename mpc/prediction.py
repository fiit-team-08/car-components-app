...
def get_straight_course(dl):
    ...
    aay, aax = transformer.transform(log.LON, log.LAT)
    abs(aay)
    abs(aax)
    aay = aay/1.25
    aax = aax/1.5
    ...
    lap = pd.read_csv("lap.csv", header=None, sep=';', names=['0', '1', 'LAT', '3', 'LON', '5', 'UTMX', '7', 'UTMY', '9', 'HMSL', '11', 'GSPEED', '13', 'CRS', '15', 'HACC', '17', 'NXPT'])
    lap = lap.drop(columns=['0', '1', '3', '5', 'UTMX', '7', 'UTMY', '9', 'HMSL', '11', '13', '15', 'HACC', '17', 'NXPT'])
    lap = lap.dropna()
    lap.LAT = lap.LAT/10000000
    lap.LON = lap.LON/10000000
    lap.GSPEED = lap.GSPEED*0.036
    lap.CRS = lap.CRS/100000
...
def main():
    ...
    lap.LON = lap.LON/10000000
    transformer = Transformer.from_crs("epsg:4326", "epsg:8353")
    y, x = transformer.transform(lap.LON, lap.LAT)
    abs(y)
    abs(x)
    y = y/1.25
    x = x/1.5
    ...
    initial_state = State(x=x[0], y=y[0], yaw=cyaw[0], v=0.0)

    t, x, y, yaw, v, d, a = do_simulation(
        cx, cy, cyaw, ck, sp, dl, initial_state)

    if show_animation2:  # pragma: no cover
        plt.close("all")
        plt.subplots()
