...
class CarRacingDQNAgent:
    def __init__(self, action_space=None, frame_stack_num=3, memory_size=5000,
    ...
    def build_model(self):
        ...
        model = Sequential()
        model.add(Conv2D(filters=6, kernel_size=(7, 7), strides=3,
                         activation='relu',
    ...
    def memorize(self, state, action, reward, next_state, done):
        self.memory.append((state, self.action_space.index(action), reward,
                            next_state, done))
    ...
    def replay(self, batch_size):
        ...
        self.model.fit(np.array(train_state), np.array(train_target),
                       epochs=1, verbose=0)
